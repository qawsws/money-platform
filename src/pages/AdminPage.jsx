...
export default function AdminPage() {
  ...
  const selectedUser = useMemo(() => {
    if (!data) return null;
    return data.users.find((entry) => entry.id === selectedUserId) || null;
  }, [data, selectedUserId]);
        ...
        <>
          <section className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-8">
              ...
              <Metric label="즐겨찾기" value={data.counts.favorites} />
          ...
          <TableSection title="커뮤니티 글 관리" empty="관리할 글이 없습니다.">
            {filtered.posts.map((post) => (
              <Row
                key={post.id}
                title={post.title}
                meta={`${post.authorName || post.author} · ${post.category} · 조회 ${post.views} · 좋아요 ${post.likes}`}
                badge={post.isHidden ? '숨김' : '공개'}
                badgeTone={post.isHidden ? 'amber' : 'green'}
                actions={(
                  <>
                    <button type="button" onClick={() => { if (window.confirm(post.isHidden ? '이 글을 다시 공개할까요?' : '이 글을 사용자 화면에서 숨길까요?')) updatePostVisibility.mutate({ id: post.id, isHidden: !post.isHidden }); }} className="rounded-md border border-amber-200 px-3 py-2 text-sm font-bold text-amber-600 hover:bg-amber-50" disabled={updatePostVisibility.isPending}>{post.isHidden ? '복구' : '숨김'}</button>
                    <button type="button" onClick={() => { if (window.confirm('이 커뮤니티 글을 삭제할까요?')) removePost.mutate(post.id); }} className="rounded-md border border-red-200 px-3 py-2 text-sm font-bold text-red-500 hover:bg-red-50" disabled={removePost.isPending}>삭제</button>
                  </>
                )}
              />
            ))}
          ...
          <TableSection title="댓글 관리" empty="관리할 댓글이 없습니다.">
            {filtered.comments.map((comment) => (
              <Row
                key={comment.id}
                title={comment.content}
                meta={`${comment.authorName || comment.author} · 글 번호 ${comment.postId} · ${comment.createdAt}`}
                actions={<button type="button" onClick={() => { if (window.confirm('이 댓글을 삭제할까요?')) removeComment.mutate(comment.id); }} className="rounded-md border border-red-200 px-3 py-2 text-sm font-bold text-red-500 hover:bg-red-50" disabled={removeComment.isPending}>삭제</button>}
              />
            ))}
          </TableSection>
        </>
...
function UserRow({ entry, currentUserId, active, selectedNote, onDetail, onNoteChange, onSaveNote, saving, onRole, rolePending, onRemove, removePending }) {
  return (
    <div className={active ? 'bg-blue-50/40' : ''}>
      <Row
        title={`${displayName(entry)} · @${entry.username}`}
        meta={`${entry.email} · ${entry.phone}`}
        badge={entry.isAdmin ? '관리자' : '사용자'}
        badgeTone={entry.isAdmin ? 'blue' : 'slate'}
        actions={(
          <>
            <button type="button" onClick={onDetail} className="rounded-md border border-slate-300 px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">{active ? '닫기' : '상세'}</button>
            {entry.id !== currentUserId && <button type="button" onClick={onRole} className="rounded-md border border-blue-200 px-3 py-2 text-sm font-bold text-blue-600 hover:bg-blue-50" disabled={rolePending}>{entry.isAdmin ? '사용자로 변경' : '관리자로 지정'}</button>}
            {entry.id !== currentUserId && <button type="button" onClick={onRemove} className="rounded-md border border-red-200 px-3 py-2 text-sm font-bold text-red-500 hover:bg-red-50" disabled={removePending}>삭제</button>}
          </>
        )}
      />
...
function UserDetail({ user, note, onNoteChange, onSave, saving }) {
  return (
    <section className="mx-4 mb-4 rounded-md border border-blue-100 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-xs font-bold text-blue-600">회원 상세</p><h2 className="mt-1 text-xl font-bold text-slate-950">{displayName(user)}</h2><p className="mt-1 text-sm text-slate-500">아이디 @{user.username} · {user.email}</p></div>
        <span className={`rounded-full px-3 py-1 text-xs font-bold ${user.isAdmin ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-600'}`}>{user.isAdmin ? '관리자' : '사용자'}</span>
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <MiniMetric label="즐겨찾기" value={user.favoritesCount} />
      ...
      <textarea id={`admin-user-note-${user.id}`} value={note} onChange={(event) => onNoteChange(event.target.value)} className="mt-2 min-h-24 w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-950 outline-none focus:border-blue-500" placeholder="회원 응대 이력, 확인할 점, 운영 메모를 남겨두세요." />
      <div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-slate-500">{user.adminNoteUpdatedAt ? `마지막 저장 ${user.adminNoteUpdatedAt}` : '저장된 메모가 없습니다.'}</p><button type="button" onClick={onSave} disabled={saving} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300">{saving ? '저장 중' : '메모 저장'}</button></div>
    </section>
  );
}
  ...
  return (
    <section className="mb-5 overflow-hidden rounded-md border border-slate-200 bg-white">
      ...
      <form onSubmit={onCreate} className="border-b border-slate-100 p-4">
        ...
        <div className="mt-3 flex justify-end"><button type="submit" disabled={saving || form.title.trim().length < 2 || form.content.trim().length < 5} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300">{saving ? '저장 중' : editing ? '수정 저장' : '공지 등록'}</button>{editing && <button type="button" onClick={onCancelEdit} className="ml-2 rounded-md border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">수정 취소</button>}</div>
      </form>
      {notices.length === 0 ? <p className="p-5 text-sm text-slate-500">등록된 공지가 없습니다.</p> : <div className="divide-y divide-slate-100">{notices.map((notice) => <div key={notice.id} className="flex items-center justify-between gap-4 px-4 py-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-1 text-xs font-bold ${notice.priority === 'important' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600'}`}>{notice.priority === 'important' ? '중요' : '일반'}</span><span className={`rounded-full px-2 py-1 text-xs font-bold ${notice.isHidden ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>{notice.isHidden ? '숨김' : '공개'}</span></div><b className="mt-2 block line-clamp-1 text-slate-950">{notice.title}</b><p className="mt-1 line-clamp-1 text-sm text-slate-500">{notice.content}</p></div><div className="flex shrink-0 flex-wrap justify-end gap-2"><button type="button" onClick={() => onEdit(notice)} className="rounded-md border border-blue-200 px-3 py-2 text-sm font-bold text-blue-600 hover:bg-blue-50">수정</button><button type="button" onClick={() => onToggle(notice)} disabled={updating} className="rounded-md border border-amber-200 px-3 py-2 text-sm font-bold text-amber-600 hover:bg-amber-50">{notice.isHidden ? '복구' : '숨김'}</button><button type="button" onClick={() => onRemove(notice)} disabled={removing} className="rounded-md border border-red-200 px-3 py-2 text-sm font-bold text-red-500 hover:bg-red-50">삭제</button></div></div>)}</div>}
    </section>
  );
}
...
function Row({ title, meta, badge, badgeTone = 'slate', actions }) {
  const badgeClasses = { amber: 'bg-amber-50 text-amber-700', blue: 'bg-blue-50 text-blue-700', green: 'bg-emerald-50 text-emerald-700', slate: 'bg-slate-100 text-slate-600' };
