      ...
      <Row
        ...
        actions={(
          <>
            ...
            {entry.id !== currentUserId && <button type="button" onClick={onRole} className="rounded-md border border-emerald-200 px-3 py-2 text-sm font-bold text-emerald-700 hover:bg-emerald-50" disabled={rolePending}>{entry.isAdmin ? '사용자로 변경' : '관리자로 지정'}</button>}
  ...
  return (
    <section className="mx-4 mb-4 rounded-md border border-emerald-100 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-xs font-bold text-emerald-700">회원 상세</p><h2 className="mt-1 text-xl font-bold text-slate-950">{displayName(user)}</h2><p className="mt-1 text-sm text-slate-500">아이디 @{user.username} · {user.email}</p></div>
        <span className={`rounded-full px-3 py-1 text-xs font-bold ${user.isAdmin ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{user.isAdmin ? '관리자' : '사용자'}</span>
      </div>
