...
const aiPercent = (value) => `${Number(value || 0).toLocaleString('ko-KR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%`;
const formatAiPercentText = (value) => String(value || '').replace(/(-?\d+(?:\.\d+)?)%/g, (_, number) => aiPercent(number));
...
function ListBox({ title, items, formatItem = (item) => item }) {
  return (
    <div className="rounded-2xl border border-[var(--color-border)] bg-slate-50 p-4">
      ...
      {items?.length > 0 ? <ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--color-text-secondary)]">{items.map((item) => <li key={item}>- {formatItem(item)}</li>)}</ul> : <p className="mt-3 text-sm text-[var(--color-text-tertiary)]">-</p>}
    </div>
  );
}
    ...
    <Card hover={false} className="p-5 sm:p-6">
      <div className="flex flex-col gap-3 border-b border-[var(--color-border)] pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--color-text-secondary)]">{formatAiPercentText(result.overallSummary)}</p>
        </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <InfoCell label="총 수익률" value={aiPercent(basis.totalReturnRate)} tone={toneClass(basis.totalProfit)} strong />
        <InfoCell label="상위 3개 비중" value={aiPercent(result.composition?.topPositionsWeight)} strong />
      </div>
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-[var(--color-border)] bg-white p-4">
          <h3 className="text-base font-extrabold text-[var(--color-text-primary)]">자산 구성 분석</h3>
          <p className="mt-2 text-sm leading-6 text-[var(--color-text-secondary)]">{formatAiPercentText(result.composition?.summary)}</p>
          {largest && <p className="mt-4 rounded-2xl bg-slate-50 p-4 text-lg font-black text-[var(--color-text-primary)]">{largest.symbol} / {largest.name} / {aiPercent(largest.weight)}</p>}
        </div>
        <ListBox title="집중도 및 위험" items={(result.risks || []).map((risk) => `${risk.title}: ${risk.description}`)} formatItem={formatAiPercentText} />
        <ListBox title="수익 구조 분석" items={[result.performance?.summary, ...(result.performance?.positiveContributors || []), ...(result.performance?.negativeContributors || [])].filter(Boolean)} formatItem={formatAiPercentText} />
        <ListBox title="AI 체크 포인트" items={result.checkpoints} formatItem={formatAiPercentText} />
      </div>
