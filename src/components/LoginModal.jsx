  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/30 px-4 py-6 backdrop-blur-sm sm:py-10">
      <div role="dialog" aria-modal="true" aria-labelledby="login-title" className="max-h-[calc(100dvh-3rem)] w-full max-w-sm overflow-y-auto rounded-[var(--radius-card)] border border-[var(--color-border)] bg-white p-6 shadow-[var(--shadow-card)]">
        ...
        {notice && <p className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-sm font-semibold text-blue-700" role="status">{notice}</p>}
        ...
        <form onSubmit={submit} className="mt-5 space-y-4">
          <label htmlFor="login-username" className="block text-sm font-bold text-[var(--color-text-primary)]">
            {t.username}
            <input id="login-username" value={username} onChange={(event) => setUsername(event.target.value)} placeholder={t.username} required autoComplete="username" className="mt-2 h-11 w-full rounded-2xl border border-[var(--color-border)] px-4 text-sm outline-none transition focus:border-[var(--color-primary)] focus:ring-4 focus:ring-blue-100" />
          </label>
          <label htmlFor="login-password" className="block text-sm font-bold text-[var(--color-text-primary)]">
            {t.password}
            <input id="login-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder={t.password} required type="password" autoComplete="current-password" className="mt-2 h-11 w-full rounded-2xl border border-[var(--color-border)] px-4 text-sm outline-none transition focus:border-[var(--color-primary)] focus:ring-4 focus:ring-blue-100" />
          </label>
